export const tourPackagesData = {
    "Tamil Nadu Tour Package": {
      title: "Tamil Nadu Tour Package",
      description: "Explore the cultural heritage of Tamil Nadu",
      features: [
        {
          id: "coimbatore",
          title: "Coimbatore",
          description:
            "Explore the capital city of Tamil Nadu with its beautiful Marina Beach, historic temples, and colonial architecture.",
          image: "/tp-features/cbe.jpeg",
          detailImages: ["/cbe/cbe-4.jpg", "/cbe/cbe-2.jpeg", "/cbe/cbe-1.jpg","/cbe/cbe-3.jpg" ],
          fullDescription:
            "Chennai, formerly known as Madras, is the capital city of Tamil Nadu and a major cultural, economic, and educational center of South India. This vibrant metropolis offers a perfect blend of tradition and modernity with its historic landmarks, beautiful beaches, and bustling markets.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival and Marina Beach",
              description:
                "Arrive in Chennai and check into your hotel. Visit Marina Beach, the second-longest urban beach in the world, and enjoy the sunset. Explore the local street food scene in the evening.",
            },
            {
              day: "Day 2",
              title: "Historic Temples",
              description:
                "Visit the ancient Kapaleeshwarar Temple in Mylapore, dedicated to Lord Shiva. Explore the San Thome Basilica, built over the tomb of St. Thomas the Apostle.",
            },
             
          ],
          
  },
        {
          id: "Ooty",
          title: "Ooty",
          description:
            "Visit the UNESCO World Heritage site featuring ancient rock-cut temples and shore temples from the 7th century.",
          image: "/tp-features/ooty.jpg",
          detailImages: ["/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg"],
          fullDescription:
            "Mahabalipuram, also known as Mamallapuram, is a historic city and UNESCO World Heritage site located on the Coromandel Coast of Tamil Nadu. Famous for its stone-carved monuments dating back to the 7th and 8th centuries, this ancient port city showcases the architectural brilliance of the Pallava dynasty.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Mahabalipuram",
              description:
                "Arrive in Mahabalipuram and check into your beachside resort. Evening visit to the Shore Temple for sunset views over the Bay of Bengal.",
            },
            {
              day: "Day 2",
              title: "Rock-Cut Monuments",
              description:
                "Explore the famous Five Rathas (monolithic temples shaped like chariots) and Arjuna's Penance, a massive open-air rock relief carving.",
            },
            {
              day: "Day 3",
              title: "Cave Temples",
              description:
                "Visit the ancient cave temples including the Varaha Cave Temple and Krishna's Butter Ball, a giant balanced boulder.",
            },
            {
              day: "Day 4",
              title: "Beach Day and Sculpture Workshop",
              description:
                "Relax at the beach in the morning. Afternoon visit to a local sculpture workshop to learn about the continuing tradition of stone carving.",
            },
           
          ],
       
        },
        {
          id: "kodaikanal",
          title: "Kodaikanal",
          description:
            "Experience the French colonial charm of this unique coastal town with its colorful streets and peaceful ashrams.",
          image: "/tp-features/kodai.jpg",
          detailImages: ["/samplw-1.jpg", "/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg"],
          fullDescription:
            "Pondicherry (now Puducherry) is a charming coastal town that was a French colonial settlement until 1954. The city's unique blend of French and Tamil cultures is evident in its architecture, cuisine, and lifestyle. With its clean beaches, spiritual centers, and colonial heritage, Pondicherry offers a tranquil escape from the hustle and bustle of city life.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Pondicherry",
              description:
                "Arrive in Pondicherry and check into your heritage hotel in the French Quarter. Evening walk along the famous Promenade Beach.",
            },
            {
              day: "Day 2",
              title: "French Quarter Exploration",
              description:
                "Guided walking tour of the French Quarter (White Town) with its colonial buildings, boutiques, and cafes. Visit the Pondicherry Museum and Sacred Heart Church.",
            },
            
          ],
          
        },
        {
          id: "Kanyakumari",
          title: "Kanayakumari",
          description:
            "Visit the magnificent Meenakshi Amman Temple and explore the ancient city's rich cultural heritage.",
          image: "/tp-features/kanyakumari.jpeg",
          detailImages: ["/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg"],
          fullDescription:
            "Madurai, one of the oldest continuously inhabited cities in the world, is the cultural capital of Tamil Nadu. Known as the 'Athens of the East', Madurai's skyline is dominated by the colorful gopurams (towers) of the Meenakshi Amman Temple. This ancient city offers visitors a glimpse into the rich religious and cultural heritage of South India.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Madurai",
              description:
                "Arrive in Madurai and check into your hotel. Evening visit to witness the night ceremony (Palliyarai) at Meenakshi Amman Temple.",
            },
            {
              day: "Day 2",
              title: "Meenakshi Temple Exploration",
              description:
                "Full day exploration of the magnificent Meenakshi Amman Temple with its 14 colorful gopurams and the Thousand Pillar Hall.",
            },
            {
              day: "Day 3",
              title: "Historic Madurai",
              description:
                "Visit Thirumalai Nayakkar Palace, Gandhi Museum, and Koodal Azhagar Temple to learn about the city's rich history.",
            },
           
          ],
         
        },
        {
          id: "valparai",
          title: "Valparai",
          description:
            "Enjoy the cool climate and scenic beauty of this popular hill station with its tea gardens and botanical gardens.",
          image: "/tp-features/valparai.jpeg",
          detailImages: ["/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg"],
          fullDescription:
            "Ooty (Udhagamandalam), known as the 'Queen of Hill Stations', is a picturesque hill station in the Nilgiri Hills of Tamil Nadu. Established as a summer retreat during British colonial rule, Ooty charms visitors with its pleasant climate, lush tea plantations, beautiful lakes, and well-maintained gardens. The hill station offers a perfect escape from the heat of the plains.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Ooty",
              description:
                "Arrive in Ooty and check into your hotel. Evening stroll around Ooty Lake with optional boating experience.",
            },
            {
              day: "Day 2",
              title: "Botanical Gardens and Tea Factory",
              description:
                "Visit the Government Botanical Gardens with its rare plant species. Tour a tea factory to learn about tea processing and enjoy tea tasting.",
            },
            {
              day: "Day 3",
              title: "Nilgiri Mountain Railway",
              description:
                "Experience the UNESCO World Heritage Nilgiri Mountain Railway (toy train) journey from Ooty to Coonoor through picturesque landscapes.",
            },
            {
              day: "Day 4",
              title: "Doddabetta Peak and Rose Garden",
              description:
                "Visit Doddabetta Peak, the highest point in the Nilgiris offering panoramic views. Explore the beautiful Government Rose Garden with over 20,000 varieties of roses.",
            },
            {
              day: "Day 5",
              title: "Pykara Lake and Falls",
              description:
                "Day trip to Pykara Lake and Waterfall. Enjoy boating on the lake and the scenic beauty of the surroundings.",
            },
            {
              day: "Day 6",
              title: "Tribal Village and Chocolate Shopping",
              description:
                "Visit a Toda tribal village to learn about the indigenous culture. Shop for Ooty's famous homemade chocolates and essential oils.",
            },
            {
              day: "Day 7",
              title: "Departure",
              description:
                "Morning visit to the Ooty Flower Show (if in season) or Wax World museum. Departure from Ooty with memories of the beautiful hill station.",
            },
          ],
         
        },
        {
          id: "BlackThunder",
          title: "Black Thunder",
          description:
            "Discover the princess of hill stations with its star-shaped lake, pine forests, and beautiful viewpoints.",
          image: "/tp-features/black-thunder",
          detailImages: ["/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg"],
          fullDescription:
            "Kodaikanal, known as the 'Princess of Hill Stations', is a charming hill retreat in the Palani Hills of Tamil Nadu. At an altitude of 7,000 feet, this hill station offers a refreshing climate, misty landscapes, and a tranquil atmosphere. With its star-shaped lake, dense forests, cascading waterfalls, and scenic viewpoints, Kodaikanal is a paradise for nature lovers and those seeking peace away from urban chaos.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Kodaikanal",
              description:
                "Arrive in Kodaikanal and check into your hotel. Evening walk around the beautiful Kodaikanal Lake with optional cycling or boating.",
            },
            {
              day: "Day 2",
              title: "Coaker's Walk and Bryant Park",
              description:
                "Morning visit to Coaker's Walk for panoramic views of the valleys. Explore Bryant Park with its colorful flower beds and plant nursery.",
            },
            {
              day: "Day 3",
              title: "Pillar Rocks and Silver Cascade",
              description:
                "Visit the magnificent Pillar Rocks, three vertically positioned boulders reaching a height of 400 feet. Stop at Silver Cascade waterfall on the way back.",
            },
          
          ],
        
        },
        {
          id: "Mudumalai",
          title: "Mudumalai",
          description:
            "Discover the princess of hill stations with its star-shaped lake, pine forests, and beautiful viewpoints.",
          image: "/tp-features/mudumalai.jpg",
          detailImages: ["/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg", "/Images/TamilNadu.jpg"],
          fullDescription:
            "Kodaikanal, known as the 'Princess of Hill Stations', is a charming hill retreat in the Palani Hills of Tamil Nadu. At an altitude of 7,000 feet, this hill station offers a refreshing climate, misty landscapes, and a tranquil atmosphere. With its star-shaped lake, dense forests, cascading waterfalls, and scenic viewpoints, Kodaikanal is a paradise for nature lovers and those seeking peace away from urban chaos.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Kodaikanal",
              description:
                "Arrive in Kodaikanal and check into your hotel. Evening walk around the beautiful Kodaikanal Lake with optional cycling or boating.",
            },
            {
              day: "Day 2",
              title: "Coaker's Walk and Bryant Park",
              description:
                "Morning visit to Coaker's Walk for panoramic views of the valleys. Explore Bryant Park with its colorful flower beds and plant nursery.",
            },
            {
              day: "Day 3",
              title: "Pillar Rocks and Silver Cascade",
              description:
                "Visit the magnificent Pillar Rocks, three vertically positioned boulders reaching a height of 400 feet. Stop at Silver Cascade waterfall on the way back.",
            },
          
          ],
        
        },
        {
          id: "Rameswaram",
          title: "Rameswaram",
          description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
          image: "/tp-features/rameswaram.jpeg",
          detailImages: ["/tp-places/rameswaram-1.jpeg", "/tp-places/rameswaram-2.jpg","/tp-places/rameswaram-3.jpg", "/tp-places/rameswaram-4.jpg","/tp-places/rameswaram-5.jpg"],
          fullDescription:
            "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Munnar",
              description:
                "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
            },
        
         
          ],
        
        },
      ],
    },
    "Kerala Tour Package": {
      title: "Kerala Tour Package",
      description: "Experience the beauty of God's Own Country",
      features: [
        {
          id: "athirappilly",
          title: "Athirappilly",
          description:
            "Cruise through the serene backwaters of Kochi and witness the traditional fishing nets and colonial architecture.",
          image: "/tp-features/athirappilly.jpeg",
          detailImages: ["/tp-places/athirappilly-1.jpeg", "/tp-places/athirappilly-2.jpeg", "/tp-places/athirappilly-3.jpg","/tp-places/athirappilly-4.jpeg"],
          fullDescription:
            "Kochi (formerly Cochin) is a vibrant city located on the southwest coast of India. Known as the 'Queen of the Arabian Sea', Kochi has been a port city since the 14th century and has attracted traders and travelers from around the world. The city's unique blend of Dutch, Portuguese, and British colonial architecture, combined with its traditional fishing nets and serene backwaters, makes it a fascinating destination.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Kochi",
              description:
                "Arrive in Kochi and check into your heritage hotel in Fort Kochi. Evening walk to see the famous Chinese fishing nets at sunset.",
            },
            {
              day: "Day 2",
              title: "Colonial Kochi",
              description:
                "Explore Fort Kochi's colonial heritage including St. Francis Church, Dutch Palace, and Jewish Synagogue. Evening Kathakali dance performance.",
            },
            {
              day: "Day 3",
              title: "Backwater Cruise",
              description:
                "Full-day cruise through the serene backwaters around Kochi. Witness local life along the waterways and enjoy traditional Kerala cuisine on board.",
            },
            {
              day: "Day 4",
              title: "Vypeen Island and Cherai Beach",
              description:
                "Ferry to Vypeen Island and visit Cherai Beach, known for its golden sands where the backwaters meet the Arabian Sea.",
            },
          
          ],
       
        },
         {
          id: "munnar",
          title: "Munnar",
          description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
          image: "/tp-features/munnar.jpeg",
          detailImages: ["/tp-places/munnar-7.jpg","/tp-places/munnar-1.jpg", "/tp-places/munnar-2.jpg","/tp-places/munnar-3.jpg","/tp-places/munnar-4.jpeg", "/tp-places/munnar-6.jpg",],
          fullDescription:
            "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Munnar",
              description:
                "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
            },
            {
              day: "Day 2",
              title: "Tea Plantations and Museum",
              description:
                "Visit the vast tea plantations and the Tea Museum to learn about the history and process of tea production. Enjoy tea tasting sessions.",
            },
            {
              day: "Day 3",
              title: "Eravikulam National Park",
              description:
                "Explore Eravikulam National Park, home to the endangered Nilgiri Tahr. Visit Top Station for panoramic views of the Western Ghats.",
            },
            {
              day: "Day 4",
              title: "Mattupetty Dam and Echo Point",
              description:
                "Visit Mattupetty Dam and enjoy boating on the lake. Continue to Echo Point and the Kundala Lake for scenic beauty.",
            },
            {
              day: "Day 5",
              title: "Spice Plantation and Waterfalls",
              description:
                "Tour a spice plantation to see cardamom, pepper, and other spices growing. Visit Attukad and Power House waterfalls.",
            },
            {
              day: "Day 6",
              title: "Trekking and Local Culture",
              description:
                "Morning trek through the Letchmi Hills. Afternoon visit to a local village to experience the culture and lifestyle of the plantation workers.",
            },
            {
              day: "Day 7",
              title: "Departure",
              description:
                "Morning photography session in the misty tea gardens. Departure from Munnar with memories of the verdant landscapes.",
            },
          ],
        
        },
        {
          id: "cochin",
          title: "Cochin",
          description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
          image: "/tp-features/kochi.jpg",
          detailImages: ["/tp-places/cochin-1.jpg", "/tp-places/cochin-2.jpg", "/tp-places/cochin-3.jpeg","/tp-places/cochin-4.jpeg","/tp-places/cochin-5.jpg",],
          fullDescription:
            "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Munnar",
              description:
                "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
            },
        
         
          ],
        
        },
        {
          id: "wayanad",
          title: "Wayanad",
          description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
          image: "/tp-features/wayanad.jpg",
          detailImages: ["/tp-places/wayanad-1.jpeg", "/tp-places/wayanad-2.jpg", "/tp-places/wayanad-3.jpg","/tp-places/wayanad-4.jpg","/tp-places/wayanad-5.jpeg",],
          fullDescription:
            "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
          itinerary: [
            {
              day: "Day 1",
              title: "Arrival in Munnar",
              description:
                "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
            },
        
         
          ],
        
        },
       ],
    },
    "Karnataka Tour Package": {
        title: "Karnataka Tour Package",
        description: "Experience the beauty of God's Own Country",
        features: [
          {
            id: "mysore",
            title: "Mysore",
            description:
              "Cruise through the serene backwaters of Kochi and witness the traditional fishing nets and colonial architecture.",
            image: "/tp-features/",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Kochi (formerly Cochin) is a vibrant city located on the southwest coast of India. Known as the 'Queen of the Arabian Sea', Kochi has been a port city since the 14th century and has attracted traders and travelers from around the world. The city's unique blend of Dutch, Portuguese, and British colonial architecture, combined with its traditional fishing nets and serene backwaters, makes it a fascinating destination.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Kochi",
                description:
                  "Arrive in Kochi and check into your heritage hotel in Fort Kochi. Evening walk to see the famous Chinese fishing nets at sunset.",
              },
              {
                day: "Day 2",
                title: "Colonial Kochi",
                description:
                  "Explore Fort Kochi's colonial heritage including St. Francis Church, Dutch Palace, and Jewish Synagogue. Evening Kathakali dance performance.",
              },
              {
                day: "Day 3",
                title: "Backwater Cruise",
                description:
                  "Full-day cruise through the serene backwaters around Kochi. Witness local life along the waterways and enjoy traditional Kerala cuisine on board.",
              },
              {
                day: "Day 4",
                title: "Vypeen Island and Cherai Beach",
                description:
                  "Ferry to Vypeen Island and visit Cherai Beach, known for its golden sands where the backwaters meet the Arabian Sea.",
              },
              
            ],
           
          },
           {
            id: "coorg",
            title: "Coorg",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/tp-features/",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "bengaluru",
            title: "Bengaluru",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/tp-features/",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
         ],
      },
"Hill Station Tour Package": {
        title: "Hills Tour Package",
        description: "Experience the beauty of God's Own Country",
        features: [
          {
            id: "Ooty",
            title: "Ooty",
            description:
              "Cruise through the serene backwaters of Kochi and witness the traditional fishing nets and colonial architecture.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Kochi (formerly Cochin) is a vibrant city located on the southwest coast of India. Known as the 'Queen of the Arabian Sea', Kochi has been a port city since the 14th century and has attracted traders and travelers from around the world. The city's unique blend of Dutch, Portuguese, and British colonial architecture, combined with its traditional fishing nets and serene backwaters, makes it a fascinating destination.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Kochi",
                description:
                  "Arrive in Kochi and check into your heritage hotel in Fort Kochi. Evening walk to see the famous Chinese fishing nets at sunset.",
              },
              {
                day: "Day 2",
                title: "Colonial Kochi",
                description:
                  "Explore Fort Kochi's colonial heritage including St. Francis Church, Dutch Palace, and Jewish Synagogue. Evening Kathakali dance performance.",
              },
              {
                day: "Day 3",
                title: "Backwater Cruise",
                description:
                  "Full-day cruise through the serene backwaters around Kochi. Witness local life along the waterways and enjoy traditional Kerala cuisine on board.",
              },
              {
                day: "Day 4",
                title: "Vypeen Island and Cherai Beach",
                description:
                  "Ferry to Vypeen Island and visit Cherai Beach, known for its golden sands where the backwaters meet the Arabian Sea.",
              },
              
            ],
           
          },
           {
            id: "kodaikanal",
            title: "Kodaikanal",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          
         ],
      },
"Temple Tour Package": {
        title: "Temple Tour Package",
        description: "Experience the beauty of God's Own Country",
        features: [
          {
            id: "esha",
            title:"Esha",
            description:
              "Cruise through the serene backwaters of Kochi and witness the traditional fishing nets and colonial architecture.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Kochi (formerly Cochin) is a vibrant city located on the southwest coast of India. Known as the 'Queen of the Arabian Sea', Kochi has been a port city since the 14th century and has attracted traders and travelers from around the world. The city's unique blend of Dutch, Portuguese, and British colonial architecture, combined with its traditional fishing nets and serene backwaters, makes it a fascinating destination.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Kochi",
                description:
                  "Arrive in Kochi and check into your heritage hotel in Fort Kochi. Evening walk to see the famous Chinese fishing nets at sunset.",
              },
              {
                day: "Day 2",
                title: "Colonial Kochi",
                description:
                  "Explore Fort Kochi's colonial heritage including St. Francis Church, Dutch Palace, and Jewish Synagogue. Evening Kathakali dance performance.",
              },
              {
                day: "Day 3",
                title: "Backwater Cruise",
                description:
                  "Full-day cruise through the serene backwaters around Kochi. Witness local life along the waterways and enjoy traditional Kerala cuisine on board.",
              },
              {
                day: "Day 4",
                title: "Vypeen Island and Cherai Beach",
                description:
                  "Ferry to Vypeen Island and visit Cherai Beach, known for its golden sands where the backwaters meet the Arabian Sea.",
              },
              
            ],
           
          },
           {
            id: "sabarimala",
            title: "Sabarimala",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Navagraha",
            title: "Navagraha",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "palani",
            title: "Palani",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Guruvayur",
            title: "Guruvayur",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "madurai",
            title: "Madurai",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Rameswaram",
            title: "Rameswaram",
            description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "marudhamalai",
            title: "Marudhamalai",
            description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "thiruvannamalai",
            title: "Thiruvannamalai",
            description:
            "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
         ],
      },
    "Combine Tour Package": {
        title: "Combine Tour Package",
        description: "Experience the beauty of God's Own Country",
        features: [
          {
            id: "honeymoon",
            title: "Honey Moon",
            description:
              "Cruise through the serene backwaters of Kochi and witness the traditional fishing nets and colonial architecture.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Kochi (formerly Cochin) is a vibrant city located on the southwest coast of India. Known as the 'Queen of the Arabian Sea', Kochi has been a port city since the 14th century and has attracted traders and travelers from around the world. The city's unique blend of Dutch, Portuguese, and British colonial architecture, combined with its traditional fishing nets and serene backwaters, makes it a fascinating destination.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Kochi",
                description:
                  "Arrive in Kochi and check into your heritage hotel in Fort Kochi. Evening walk to see the famous Chinese fishing nets at sunset.",
              },
              {
                day: "Day 2",
                title: "Colonial Kochi",
                description:
                  "Explore Fort Kochi's colonial heritage including St. Francis Church, Dutch Palace, and Jewish Synagogue. Evening Kathakali dance performance.",
              },
              {
                day: "Day 3",
                title: "Backwater Cruise",
                description:
                  "Full-day cruise through the serene backwaters around Kochi. Witness local life along the waterways and enjoy traditional Kerala cuisine on board.",
              },
              {
                day: "Day 4",
                title: "Vypeen Island and Cherai Beach",
                description:
                  "Ferry to Vypeen Island and visit Cherai Beach, known for its golden sands where the backwaters meet the Arabian Sea.",
              },
              
            ],
           
          },
          {
            id: "Navagraha",
            title: "Navagraha",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
           {
            id: "leisure",
            title: "Leisure",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "bangalore&Mysore",
            title: "Bangalore & Mysore",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "kerala",
            title: "Kerala",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Ooty&Wayanad",
            title: "Ooty & Wayanad",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Ooty&Coorg",
            title: "Ooty & Coorg",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Palani&madurai",
            title: "Palani & Madurai",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
          {
            id: "Ooty&Kerala",
            title: "Ooty&Kerala",
            description:
              "Explore the lush green tea plantations and enjoy the cool climate of this beautiful hill station.",
            image: "/Images/kerala.jpg",
            detailImages: ["/Images/kerala.jpg", "/Images/kerala.jpg", "/Images/kerala.jpg"],
            fullDescription:
              "Munnar, situated at the confluence of three mountain streams, is a picturesque hill station in Kerala's Western Ghats. Once a summer resort of the British government in South India, Munnar is famous for its vast expanse of tea plantations, pristine valleys, and cool mountain air. The hill station stands at 1,600 meters above sea level, offering breathtaking views and a pleasant climate throughout the year.",
            itinerary: [
              {
                day: "Day 1",
                title: "Arrival in Munnar",
                description:
                  "Arrive in Munnar and check into your mountain resort. Evening walk through the nearby tea gardens to enjoy the sunset views.",
              },
          
           
            ],
          
          },
         
          
         ],
      },
    
    }
  
  